#!/usr/bin/env python3
"""ByteClub Content Repository Validation Script.

Run locally to validate entries before pushing.
Same logic as GitHub Actions validate.yml.
"""
...
def load_schema():
    ...
    schema_path = os.path.join(os.path.dirname(__file__), "..", "schema", "entry.schema.json")
...
def validate_entry(entry_path, schema):
    ...
    if not os.path.isfile(readme_path):
        ...
        # Still continue to check other things, but markdown this as critical

    # 3. Check directory name == slug
    dir_name = os.path.basename(entry_dir)
    slug = data.get("slug") if "data" in locals() else None
    # Re-load slug if we don't have it
    if slug is None:
        with open(yaml_path) as f:
            data = yaml.safe_load(f)
            slug = data.get("slug", "")
    ...
    # 12. Check for duplicate slugs (across all entries)
    # This is handled at the repo level, so we just return per-entry errors
